    ...
    <TotalSpentWrap>
      ...
      <div className="total-spent-content">
        <div className="total-spent-summary">
          <div className="spent-summary-val">55.43PHP</div>
          <div className="spent-summary-info">
            ...
            <div className=" spent-summary-statdata">
              ...
              <span className="statdata-value">5.32PHP</span>
            </div>
          </div>
        ...
        <div className="line-chart-block">
          <ResponsiveContainer width = "100%" height = "100%">
            <LineChart data = {TOTAL_SPENT_DATA} margin = {{
              top: 10, right: 30, left: 0, bottom:20
            }}>
              <XAxis dataKey = "month" tickSize={0} axisLine={false} padding = {{ left: 20 }} tick = {({ payload, x, y, dy}) => (
                <text x = {x} y = {y + 20} dy = {dy} 
                textAnchor="middle" fill={theme.colors.powderBlue} fontSize={12} fontWeight={500}>{payload.value.toUpperCase()}</text> 
              )} />
              <YAxis tickFormatter={formatYAxisLabel} tickSize={6} axisLine = {false} ticks={[0, 10, 20, 30, 40, 50, 60]} tick = {{
                fontSize: 12, fill: theme.colors.powderBlue, fontWeight: 500
              }} interval={0} />
              <Line type="monotone" dataKey = "uv" stroke={theme.colors.majorelleBlue} strokeWidth={2} dot = {{ strokeWidth: 3, fill: theme.colors.majorelleBlue, r: 6 }} />
              <Line type="monotone" dataKey = "pv" stroke={theme.colors.pictonBlue} strokeWidth={2} dot = {{ strokeWidth: 3, fill: theme.colors.majorelleBlue, r: 6 }} />
